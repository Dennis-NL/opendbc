...
unsigned int volkswagen_mxb_checksum(uint32_t address, const Signal &sig, const std::vector<uint8_t> &d) {
  ...
  switch (address) {
    ...
    case 0x65D: // ESP_20 Electronic Stability Program
      ...
      break;
    case 0x130: // PLA_01
      crc ^= (uint8_t[]){0xAB, 0xCD, 0xEF, 0x01, 0x23, 0x45, 0x67, 0x89, 0x10, 0x32, 0x54, 0x76, 0x98, 0xBA, 0xDC, 0xFE}[counter];
      break;
...
unsigned int xor_checksum(uint32_t address, const Signal &sig, const std::vector<uint8_t> &d) {
  ...
  switch(address){
    case 0x130: // mPLA_1, TX from OP
      ...
      break;
    default:    // Simple XOR over the payload, except for the byte where the checksum lives.
      for (int i = 0; i < d.size(); i++) {
        if (i != checksum_byte) {
          checksum ^= d[i];
        }
